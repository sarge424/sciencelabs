...
if (!empty($result) && !empty($itemname)) {
	...
	while ($row = $result->fetch_assoc()) {
		if (startsWith(strtolower($row['item_name']), strtolower($itemname))) {
			$html = $html . '<div class="btn btn-block btn-light text-left" style="margin: -5px" onclick="setItem(\'' . $row['item_name'] . '\', ' . $row['id'] . ')"><b>' . $row['item_name'] . '  (' . $row['quantity'] . ')</b></div>   ' . $row['specs'] . '<br>';
			$rows += 1;
			$lastid = $row['id'];
		} else {
		}
